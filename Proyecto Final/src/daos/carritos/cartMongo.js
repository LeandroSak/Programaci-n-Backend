...
class Cart {
  ...
  async save(time, userId) {
    ...
    try {
      ...
      if (data.length != 0) {
        ...
        data = await cartModel.insertMany({
          id: ultimo.id + 1,
          timestamp: time,
          status: "open",
          userID: userId,
  ...
  async putById(numberID, prop) {
    try {
      ...
      if (data) {
        let indexP = data[0].productos.findIndex((element) =>
          element.id == prop.id
        );
        if (indexP != -1) {
          const totalcant = data[0].productos[indexP].cantidad;
          await cartModel.updateOne({ id: numberID }, {
            $set: { "productos.$[prod].cantidad": totalcant + 1 },
          }, { arrayFilters: [{ "prod.id": prop.id }] });
        } else {
          let prod = {
            title: prop.title,
            price: prop.price,
    ...
    try {
      ...
      if (data) {
        ...
        for (let i = 0; i < data[0].productos.length; i++) {
          total += `<li>${data[0].productos[i].title} cantidad: ${
            data[0].productos[i].cantidad
          }</li>`;
        }
        const mailOptions = {
          ...
          html:
            `<h1>Nuevo compra</h1><br><p>Nombre: ${userData.name}</p><p>Email: ${userData.username}</p><p>Direccion: ${userData.adress}</p><p>Edad: ${userData.age}</p><p>Telefono: ${userData.phone}</p><p>Productos: </p><ul>${total}</ul>`,
        };
