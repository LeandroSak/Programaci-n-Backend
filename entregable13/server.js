...
if (data.modo== "cluster") {
    if (Number.isInteger(data.i) && data.i > 1) {
        if (cluster.isMaster) {
            ...
            http.listen(PORT, () => console.info(`Server cluster up and running on port ${PORT}, pid: ${process.pid}`))
        }
    } else {
        if (cluster.isMaster) {
    ...
    http.listen(PORT, () => console.info(`Server fork up and running on port ${PORT}, pid: ${process.pid}`))
}
